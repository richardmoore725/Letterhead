...
class LetterDeltaParserListFormatter extends BlockListener
{
    ...
    public function render(Lexer $lexer)
    {
        ...
        foreach ($this->picks() as $pick) {
            ...
            // defines whether this attribute list element is the last one of a list serie. li
            $isLast = false;
            ...
            if (!$isOpen) {
                $output .= '<mj-text><' . $this->getListAttribute($pick) . '>';
                $isOpen = true;
            }
            ...
            if (($isOpen && $isLast) || ($isOpen && $pick->isLast())) {
                $output .= '</' . $this->getListAttribute($pick) . '></mj-text>';
                $isOpen = false;
            }
